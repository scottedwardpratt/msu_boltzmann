...
void CMSU_Boltzmann::AddAction_Activate(CMSUPart *part){
	...
	if(BJORKEN && fabs(part->eta)>ETAMAX){
		snprintf(message,CLog::CHARLENGTH,"CMSU_Boltzmann::AddAction_Activate, eta out of bounds, =%g\n",fabs(part->eta));
		CLog::Fatal(message);
	}
	action=GetDeadAction();
	if(action->currentmap==&ActionMap){
		snprintf(message,CLog::CHARLENGTH,"don't even try, action wasn't dead, key=%d\n",int(action->key));
		CLog::Fatal(message);
	}
	action->type=0;
